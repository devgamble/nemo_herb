...
lat* = latitude  
long* = longitude  
el = elevation (ClimateNA corrects climate data for elevation - make sure elevation is accurate before querying data) (in meters, NAs/empty cells allowed)  
* = required  
...
Creating a ClimateNA-compatible csv from a specimen dataframe input: (see cch1/cch2 specific scripts)  
<br>  
...
st_data <- read_csv(here("climateNA", "cch1_cna", "nemo_cch1_std.csv"))


#Import time series data - annual climate for all years for each specimen
allA_data <- read_csv(here("climateNA", "cch1_cna", "nemo_cch1_std_1901-2018Y.csv"))
```
...
norm1 <- read_csv(here("climateNA", "cch1_cna", "nemo_cch1_std_Normal_1951_1980Y.csv"))
colnames(norm1)[6:28] <-  rename_ftn1(norm1, "_51-80", 6:28)


norm2 <- read_csv(here("climateNA", "cch1_cna", "nemo_cch1_std_Normal_1981_2010Y.csv"))
...
write_csv(YoC_norms, here("climateNA", "cch1_cna", "YoC_A_norms51-10.csv"))
```
