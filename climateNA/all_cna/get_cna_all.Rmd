...
Calculate anomalies for long-term average climate *and/or* climate normals - compare later.  
...
```{r}
#Creating Anomalies for the 100Y average
#Just Temperature and PPT anomalies (for now)


#Select subset of df with longterm avg (100Y) and YoC to make anomalies
climNA_nemo_all1 <- climNA_nemo_merge %>% 
  select(specimen_number:elev_m, Tmax01_100Y:PPT12_100Y, Tmax_wt_100Y:PPT_at_100Y, MAT_100Y:MSP_100Y, Tmax01_Y:PPT12_Y, Tmax_wt_Y:PPT_at_Y, MAT_Y:MSP_Y) %>% 
  filter(!is.na(MAT_Y)) #remove records without collection year climate
#1764 obs - 90 = 1674, good

climNA_nemo_all2 <- climNA_nemo_all1 %>% 
...
#Yes, there is probably an easier way than individually mutating these, but I spent too much time trying to figure it out.
  
#  mutate_at(vars(Tmax01_Y:PPT12_Y, Tmax_wt_Y:PPT_at_Y, MAT_Y:MSP_Y), - vars(Tmax01_100Y:PPT12_100Y, Tmax_wt_100Y:PPT_at_100Y, MAT_100Y:MSP_100Y) ) #create anomalies, but they retain YoC names
