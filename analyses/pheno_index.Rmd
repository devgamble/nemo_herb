...
Note: These csvs last updated 8-24-2021 -- wille need to be REVIEWED AGAIN to clean up scores in google sheets prior to another download...
...
sum(is.na(cch2_pi_full$open_flr_present)) #0

#For PI scores
sum(is.na(cch2_pi_full$plant_PI)) #58
...
```{r}
#cch2_spec_pi <- cch2_pi_full %>% 
#  filter/mutate
#  group_by(specimen_number)

#Total number of specimens in this data
nemo_specimens <- cch2_pi_full %>% group_by(specimen_number) %>% 
  summarize(specimen_number = unique(specimen_number))
##1136 specimens


#Individual Specimens with 0 open flowers (these SHOULD BE REMOVED from data set!)
open_flr_check <- cch2_pi_full %>% group_by(specimen_number) %>% 
  summarize(has_flowers = case_when(any(open_flr_present == "y") ~ "y"))# %>% 
##1136 rows returned - all have open flowers!!!!
