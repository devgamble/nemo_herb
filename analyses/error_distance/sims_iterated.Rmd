...
summ(MLA_lm1, scale = TRUE, vifs = TRUE, confint = TRUE, ci.width = 0.95)$coeftable[2:8,2]
...
#Cannot take mean of confidence intervals-- too much variation among replicated parameter estimates
...
#Decpricated...? Not compatible with original rec results
...
##Alternative method for calculating confidence intervals???

# Confint() [boot CIs] ?? but only works on boot objects
...
ggplot(data = PCMs_avg_all) +
  geom_point(aes(x = mean_param_est, y = predictor, group = sim_dist, color = sim_dist), position = position_dodge(width = -.8), size = 2, pch = 0, stroke = 1.5) + 
  geom_errorbarh(aes(y = predictor, xmin = mean_ci_low, xmax = mean_ci_high, group = sim_dist, color = sim_dist), position = position_dodge(width = -.8), size = 1, height = 0) +
  geom_vline(xintercept = 0, lty = 2) +
  scale_color_manual(values = c("#781C81", "#447CBF", "#83BA6D", "#DBAB3B", "#D92120")) +
  theme_classic()
...
Option: Use same manual method - should be similar, will just take longer

**Load Raster files from ClimateNA**  

```{r eval = FALSE}
library(raster)
##Obtain DEM input raster file (.asc format) for California

MAT_81 <- raster('filename.asc');mat
...
```{r eval = FALSE}
#Primary wd for this project: "C:/Users/Devin/Documents/R_files/Data/nemo_herb"

#see 'system2' documentation for executing system commands

setwd(C:/Users/Devin/Desktop)

exe <- "Climatena_v7.21 - Shortcut.exe"
...
**Bootstrap confidence intervals**  
 ...
 - Should incorporate individual estimates and estimate SEs!


OR

**Bootstrap simulated deviations used in estimating phen-sen**  
