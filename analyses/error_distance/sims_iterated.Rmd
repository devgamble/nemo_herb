...
SEchange <- ggplot(data = par_est_change2) + 
  ...
  theme_bw(base_size = 18) + theme(legend.position = c(0.88, 0.7), legend.background = element_rect(colour = 'black', fill = 'white', linetype='solid', size = .5), legend.key.size = unit(.5, "cm")) #+
  #geom_text(x = .8, y = .35, label = "(a)", size = 8)
...
##Combine Panels
Figure3 <- ggarrange(SEchange, R2change, ncol = 2, labels = c("(a)", "(b)"), font.label = list(size = 18)) 
Figure3
...
nemo_combinedsims1 <- bind_rows("Original" = nemo_e2, "2km" = sims_2km_full_200, "5km" = sims_5km_full_200, "15km" = sims_15km_full_200, "25km" = sims_25km_full_200, .id = "data_origin")
...
nemo_combinedsims2 <- nemo_combinedsims1 %>% 
  ...
  mutate(data_origin = factor(data_origin, ordered = TRUE, levels = c("Original", "2km", "5km", "15km", "25km"))) %>% #level order
