---
title: "Bias and Error in Herbarium data: Error Distance Analyses"
author: "Devin Gamble"
date: "7/28/2020"
...
Only significant interactions are shown although interactions among climate variables were tested for each All Records model.  
#### 100-Y MAT & MAP
```{r message = FALSE, warning = FALSE}
##
#MAT & MAP
...
plot_summs(M100Y_lm1, er_an0, er_an1, er_an2, model.names = c("All Records", "With error", "0-5 km", "0-2 km"), scale = TRUE) + theme_bw()
...
plot_summs(M100Y_lm2, erc_an0, erc_an1, erc_an2, model.names = c("All records", "With error", "0-5 km", "0-2 km"), coefs = c("MAT" =  "MAT_100Y", "MAP" = "MAP_100Y"), legend.title = "Model Data", scale = TRUE) + theme_apa() + theme(legend.title = element_text("Model Data", size = 10), legend.text = element_text(size = 9))+ labs(y = "Predictor")
...
plot_summs(M100Y_lm1A, er_an0A, er_an1A, er_an2A, model.names = c("All Records", "With error", "0-5 km", "0-2 km"), scale = TRUE) + theme_bw()
ggsave(here::here("analyses", "figs1", "error_d_subset_MAT-MAP1A.png"), width = 6, height = 4)
#scale = TRUE refits model to mean-center & standardize the predictors but not the response. Use 'center = TRUE' to only mean center variables
summary(M100Y_lm1A) # N = 1677
...
plot_summs(M100Y_lm2A, erc_an0A, erc_an1A, erc_an2A, model.names = c("All records", "With error", "0-5 km", "0-2 km"), coefs = c("MAT" =  "MAT_A", "MAP" = "MAP_A"), legend.title = "Model Data", scale = TRUE) + theme_apa() + theme(legend.title = element_text("Model Data", size = 10), legend.text = element_text(size = 9)) + labs(y = "Predictor")
...
plot_summs(M100Y_lm1AL, er_an0AL, er_an1AL, er_an2AL, model.names = c("All records", "With error", "0-5 km", "0-2 km"), scale = TRUE) + theme_bw()
ggsave(here::here("analyses", "figs1", "error_d_subset_MAT-MAP1AL.png"), width = 6, height = 4)
#scale = TRUE refits model to mean-center & standardize the predictors but not the response. Use 'center = TRUE' to only mean center variables
summary(M100Y_lm1AL) # N = 1677
...
plot_summs(M100Y_lm2AL, erc_an0AL, erc_an1AL, erc_an2AL, model.names = c("All records", "With error", "0-5 km", "0-2 km"), scale = TRUE) + theme_bw()
ggsave(here::here("analyses", "figs1", "error_d_subset_MAT-MAP2AL.png"), width = 6, height = 4)
summary(M100Y_lm2AL)
summary(erc_an0AL)$r.squared
summary(erc_an1AL)$r.squared
summary(erc_an2AL)$r.squared

summ(M100Y_lm2AL, vif = TRUE)

#Corrplot for supp mat?
corrplot(cor(nemo_df2 %>% select(MAT_100Y, MAP_100Y, MAT_A, MAP_A, elev_m, long, lat, year)), addCoef.col = "black", number.cex = .6,) #some autocorrelation in T anoms? elev*long corr
#similar VIFs as in 100Y only models, but these are still pretty low
```
*NOTE*: Effect of year becomes nonsignificant when anomalous conditions are in model.  
<br>  
...
visreg(M100Y_lm2, "MAT_100Y", xlab = "MAT (1901-2000)", points = list(col = "#404040"), gg = TRUE) + 
  theme_apa()
ggsave(here::here("analyses", "figs1", "errorD", "DOY-MAT_100.png"), height = 4, width = 6) #Dims: 4x6 clearer than 5x8
visreg(M100Y_lm2, "MAP_100Y", xlab = "log[MAP] (1901-2000)", points = list(col = "#404040"), gg = TRUE) +  #option: use xtrans = exp 
  theme_apa()
ggsave(here::here("analyses", "figs1", "errorD", "DOY-MAP_100.png"), height = 4, width = 6)

visreg(M100Y_lm2A, "MAT_A", xlab = "MAT (Anomaly)", points = list(col = "#404040"), gg = TRUE) + 
  geom_vline(xintercept = 0, lty = 2) +
  theme_apa()
ggsave(here::here("analyses", "figs1", "errorD", "DOY-MAT_A.png"), height = 4, width = 6)
visreg(M100Y_lm2A, "MAP_A", xlab = "MAP (Anomaly)", points = list(col = "#404040"), gg = TRUE) + 
  geom_vline(xintercept = 0, lty = 2) +
  theme_apa()
...
**Focus on MAT/MAP - added anomalies**  
...
plot_summs(M100Y_lm1, M100Y_lm3a, M100Y_lm3b, model.names = c("All Records", "numeric", "factor-bins"), scale = TRUE) + theme_bw()
ggsave(here::here("analyses", "figs1", "error_covar_plotsumms_MAT-MAP1.png"), width = 6, height = 4)


#Anomalies
M100Y_lm3aA <- lm(DOY ~ MAT_A + MAP_A + error_dist_m, data = nemo_df2)
M100Y_lm3bA <- lm(DOY ~ MAT_A + MAP_A + error_bin, data = nemo_df2)

summary(M100Y_lm3aA)
summary(M100Y_lm3bA)

plot_summs(M100Y_lm1A, M100Y_lm3aA, M100Y_lm3bA, model.names = c("All Records", "numeric", "factor-bins"), scale = TRUE) + theme_bw()
#ggsave(here::here("analyses", "figs1", "error_covar_plotsumms_MAT-MAP1.png"), width = 6, height = 4)
...
plot_summs(M100Y_lm2, M100Y_lm4a, M100Y_lm4b, model.names = c("All Records", "numeric", "factor/bin"), scale = TRUE) + theme_bw()


#Other predictors + Anoms
M100Y_lm4aAL <- lm(DOY ~ MAT_100Y + MAP_100Y + MAT_A + MAP_A + elev_m + long + year + error_dist_m, data = nemo_df2)
M100Y_lm4bAL <- lm(DOY ~ MAT_100Y + MAP_100Y + MAT_A + MAP_A + elev_m + long + year + error_bin, data = nemo_df2)

summary(M100Y_lm4aAL)
summary(M100Y_lm4bAL)


plot_summs(M100Y_lm2AL, M100Y_lm4aAL, M100Y_lm4bAL, model.names = c("All Records", "numeric", "factor/bin"), scale = TRUE) + theme_bw()
...
- Eval set to FALSE. Not including in results...  
...
plot_summs(Wt100Y_lm1, Wt100Y_lm3a, Wt100Y_lm3b, model.names = c("All Records", "numeric", "factor-bins"), scale = TRUE) + theme_bw()
ggsave(here::here("analyses", "figs1", "error_covar_plotsumms_Wt1.png"), width = 6, height = 4)
...
plot_summs(Wt100Y_lm1i, Wt100Y_lm3ai, Wt100Y_lm3bi, model.names = c("All Records", "numeric", "factor/bin"), scale = TRUE) + theme_bw()
ggsave(here::here("analyses", "figs1", "error_covar_plotsumms_Wt1i.png"), width = 6, height = 4)
...
plot_summs(Wt100Y_lm2, Wt100Y_lm4a, Wt100Y_lm4b, model.names = c("All Records", "numeric", "factor/bin"), scale = TRUE) + theme_bw()
...
plot_summs(Sp100Y_lm1, Sp100Y_lm3a, Sp100Y_lm3b, model.names = c("All Records", "numeric", "factor-bins"), scale = TRUE) + theme_bw()
ggsave(here::here("analyses", "figs1", "error_covar_plotsumms_Sp1.png"), width = 6, height = 4)
...
plot_summs(Sp100Y_lm1i, Sp100Y_lm3ai, Sp100Y_lm3bi, model.names = c("All Records", "numeric", "factor-bins"), scale = TRUE) + theme_bw()
ggsave(here::here("analyses", "figs1", "error_covar_plotsumms_Sp1i.png"), width = 6, height = 4)
...
plot_summs(Sp100Y_lm2, Sp100Y_lm4a, Sp100Y_lm4b, model.names = c("All Records", "numeric", "factor/bin"), scale = TRUE) + theme_bw()
